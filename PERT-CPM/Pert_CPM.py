...
class Activity:
    """
        ...
        -------------------
        representa los bordes entre cada nodo
        Cada actividad tiene un nombre único y su duración en el proyecto.
        Las actividades serán iguales si su nombre y su duración son iguales
    """

    @log_with_msg("Actividad de inicialización")
    def __init__(self, name, duration):
        self._name = name
        self._duration = duration

    @log_with_msg("Retorno de actividad repr")
    def __repr__(self) -> str:
        return f"<{self.name}, {self.duration} semanas>"

    @log_with_msg("Actividad de retorno str")
    ...
    @log_with_msg("Comparación de actividades")
...
class Node:
    """
        La clase Nodo
        --------------
        representa el nodo real en el gráfico.
        conoce los tiempos de finalización temprano y tardío y también conoce el tiempo de inactividad
        Cada nodo es único y se reconoce por su número.
        Un nodo es igual a otro nodo si su número es igual (independientemente de las otras propiedades)
        Está configurado de esa manera para mantener los nodos únicos
        Cada nodo tiene un nodo paralelo opcional. Si un nodo tiene un nodo paralelo,
        Ambas actividades que conducen a esos nodos deben completarse juntas
    """

    @log_with_msg("Nodo de inicialización")
    def __init__(self, number: int, *parallel_nodes: "Lista de nodos"):
        self._number = number
    ...
    @log_with_msg("Repr. De nodo devuelto")
    def __repr__(self) -> repr:
        return f"(Nodo {self.number})"

    @log_with_msg("Devolviendo Node str")
    def __str__(self) -> str:
        string = f"(Nodo {self.number})"
    ...
    @log_with_msg("Comprobando si Node tiene nodos paralelos")
    def has_parallel_nodes(self) -> bool:
        return list(self.parallel_nodes) != []

    @log_with_msg("Comparación de nodos")
    def __eq__(self, other) -> bool:
    ...
    @log_with_msg("Nodo hash")
    def __hash__(self) -> float:
        return hash(self.number)

    # For sorting purposes
    @log_with_msg("Comprobando qué nodo es más grande")
...
class Transition:
    """
        Clase de transición
        ----------------
        representa las transiciones de un nodo a otro.
        realiza un seguimiento de la actividad, el nodo de inicio y el nodo de destino
    """

    @log_with_msg("Inicialización de la transición")
    def __init__(self, from_node: Node, activity: Activity, to_node: Node):
        self._from_node = from_node
        self._activity = activity
        self._to_node = to_node

    @log_with_msg("Retorno de transición de regreso")
    def __repr__(self) -> repr:
        return f"({repr(self._from_node)}, {self._activity}, {repr(self._to_node)})"

    @log_with_msg("Devolución de la cadena de transición")
    def __str__(self) -> str:
    ...
    @log_with_msg("Comparación de transiciones")
    def __eq__(self, other) -> bool:
...
class Project:
    ...
    La clase Pert
    --------------
    La clase que representa al pert, usando una gráfica.
    El gráfico es un diccionario de {Node: list (Transition)}: cada nodo con las transiciones correspondientes.
    Si no se pasó ningún gráfico al constructor, se inicializa un gráfico vacío
    """

    @log_with_msg("Inicializando nuevo PERT")
    ...
    @log_with_msg("Impresión PERT")
    def __str__(self) -> str:
        string = '!!WARNING: Invalid Graph!!' if not self.is_valid() else ''
        for path in self.all_paths:
            string += f"\nCAMINO CRITICO: " if path in self.critical_paths else f"\n" + "\t" * 3 + " " * 3
    ...
    @log_with_msg("Actualizando PERT ")
    def update(self):
    ...
    @log_with_msg("Duración de retorno de PERT")
    ...
    @log_with_msg("Añadiendo actividad ")
    ...
    def add_activities(self, *args: "Lista de transiciones"):
    ...
    @log_with_msg("Eliminando actividad ")
    ...
    @log_with_msg("Obteniendo lista de actividades")
    ...
    @log_with_msg("Obteniendo nodos aislados")
    ...
    @log_with_msg("Obteniendo rutas críticas")
    ...
    @log_with_msg("Conseguir los nodos de inicio")
    ...
    @log_with_msg("Obteniendo el nodo final")
    ...
    @log_with_msg("Cálculo de acabados tempranos")
    ...
    @log_with_msg("Cálculo de finales tardíos ")
    ...
    @log_with_msg("Calcular tiempos de inactividad")
    ...
    @log_with_msg("Encontrar todos los caminos")
    ...
    @log_with_msg("Comprobando si el artículo está en PERT")
    def __contains__(self, item) -> bool:
        if not (isinstance(item, Node) or isinstance(item, Activity) or isinstance(item, Transition)):
            raise PermissionError("Este artículo no pertenece al pert!")
...
class TestPert(unittest.TestCase):
    ...
    transition_list = [
        Transition(node_list[1], Activity('Formalizar especificaciones', 6), node_list[2]),
        Transition(node_list[2], Activity('Sistema de diseño', 4), node_list[3]),
        Transition(node_list[2], Activity('Requisitos de certificación', 3), node_list[4]),
        Transition(node_list[2], Activity('Software de diseño', 6), node_list[6]),
        Transition(node_list[3], Activity('Sistema de prototipos ', 2), node_list[5]),
        Transition(node_list[4], Activity('Documentación de certificación', 4), node_list[9]),
        Transition(node_list[9], Activity('Solicitud de certificación', 1), node_list[10]),
        Transition(node_list[10], Activity('Certificación completa', 4), node_list[11]),
        Transition(node_list[6], Activity('Software de código ', 4), node_list[8]),
        Transition(node_list[8], Activity('Software completo', 1), node_list[11]),
        Transition(node_list[5], Activity('Sistema de prueba', 3), node_list[6]),
        Transition(node_list[6], Activity('Sistema de liberación', 2), node_list[7]),
        Transition(node_list[7], Activity('Sistema de fabricación', 4), node_list[11]),
    ]
    ...
    def test_add_activity(self):
        self.pert.add_activity(11, Activity("Actividad de prueba", 2), 12)
        self.assertEqual(12, self.pert.__get_end_node__().number)
        self.assertEqual(len(self.pert), 23)

    def test_del_activity(self):
        self.pert.del_activity(Activity('Software de diseño', 6))
        self.assertNotIn([1, 2, 6, 8, 11], [[node.number for node in path] for path in self.pert.all_paths])
        self.pert.add_activity(2, Activity('Software de diseño', 6), 6)
        self.pert.del_activity(Activity('Formalizar especificaciones', 6))
    ...
    def test_invalid_graph(self):
        graph = {Node(1): [Transition(Node(1), Activity("Prueba1", 1), Node(2))],
                 Node(2): [Transition(Node(2), Activity("Prueba2", 2), Node(3))],
                 Node(3): [Transition(Node(3), Activity("Prueba3", 3), Node(4))],
                 Node(4): [Transition(Node(4), Activity("Prueba4", 4), Node(2))]}
...
class TestPert2(unittest.TestCase):
    ...
    transition_list_2 = [
        Transition(node_list_2[0], Activity("Tarea 1", 4), node_list_2[1]),
        Transition(node_list_2[0], Activity("Tarea5", 6), node_list_2[2]),
        Transition(node_list_2[0], Activity("Tarea9", 5), node_list_2[3]),
        Transition(node_list_2[2], Activity("Tarea2", 2), node_list_2[4]),
        Transition(node_list_2[3], Activity("Tarea6", 4), node_list_2[6]),
        Transition(node_list_2[4], Activity("Tarea8", 5), node_list_2[5]),
        Transition(node_list_2[4], Activity("Tarea10", 8), node_list_2[8]),
        Transition(node_list_2[4], Activity("Tarea3", 2), node_list_2[7]),
        Transition(node_list_2[5], Activity("Tarea4", 5), node_list_2[8]),
        Transition(node_list_2[6], Activity("Tarea7", 6), node_list_2[8]),
    ]
    ...
    def test_add_new_activity_to_graph_two(self):
        self.pert.add_activity(8, Activity("Tarea12", 5), 9)
    ...
    def test_graph_two_del_activity(self):
        self.pert.del_activity(Activity('Tarea3', 2))
...
def read():
    with open('HW1_Nofar_Pert_CPM.log') as f:
        print("contenido del archivo de registro:")
        print(f.read())
